...
export function AuthProvider({ children }: { children: React.ReactNode }) {
            ...
            .then(({ data: { session } }) => {
                ...
                if (session?.user) {
                    ...
                    supabase
                        ...
                        .then(({ error }) => {
                            if (error) {
                                logger.error('AuthContext', 'Error updating timezone', error)
                            }
                        })
                }
            })
            .catch((error) => {
                logger.error('AuthContext', 'Failed to get initial session', error)
                setLoading(false)
            })
    ...
    const signOut = useCallback(async () => {
        ...
        if (error) {
            logger.error('AuthContext', 'Failed to sign out', error)
            return
        }
        setUser(null)
        setSession(null)
    ...
    const completeRecovery = useCallback(() => setIsRecovery(false), [])

    const contextValue = useMemo(
        () => ({ user, session, loading, signOut, onlineUsers, isRecovery, completeRecovery }),
        [user, session, loading, signOut, onlineUsers, isRecovery, completeRecovery]
    )

-- Migration: Force Position Throwbacks (Testing)
-- Retains Challenge Media Aggregation fix
-- Forces return of only 'position' type memories
...
BEGIN
    ...
    all_memories AS (
        -- CHANGED: Force filtering to ONLY 'position' type
        SELECT * FROM all_memories_raw amr WHERE amr.type = 'position'
    ),
    numbered_items AS (
        SELECT 
            am.*, 

-- Migration: Fix get_on_this_day_contents and get_random_throwback to include media_url for photo memories
-- The RPC functions currently only return media_urls (array) but photo memories store images in media_url (singular string)
-- This migration updates both functions to coalesce media_url into the media_urls array if media_urls is null

-- Fix get_random_throwback to return media_url as media_urls for photo memories
...
BEGIN
    ...
    WITH all_memories AS (
        ...
        SELECT 
            ...
            m.created_at,
            -- Coalesce: use media_urls if it exists, otherwise wrap media_url in array
        ...
        WHERE f.couple_id = p_couple_id
            ...
            AND (p_exclude_date IS NULL OR DATE(f.completed_at) != p_exclude_date)
        
        UNION ALL
        
        -- Redeemed vouchers/coupons
        SELECT 
            c.id,
        ...
        WHERE c.couple_id = p_couple_id
            ...
            AND (p_exclude_date IS NULL OR DATE(c.redeemed_at) != p_exclude_date)
        ...
        WHERE e.couple_id = p_couple_id
            ...
            AND (p_exclude_date IS NULL OR DATE(e.event_date::timestamptz) != p_exclude_date)
    )
    SELECT * FROM all_memories
    ORDER BY random() -- Use setseed for consistent daily random
    LIMIT 1;
END;
$$;

-- Fix get_on_this_day_contents similarly

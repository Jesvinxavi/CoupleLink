-- Migration: Fix Challenge Media Aggregation (Final)
-- Aggregates media_urls from both partners for a challenge memory
-- Removes temporary "Lego" filter but keeps 'challenge' type filter for continued testing
...
BEGIN
    ...
    WITH all_memories_raw AS (
        ...
        SELECT 
            ...
            m.caption as content,
            m.created_at,
            -- CHANGED: Aggregate media from partner if available
            (
    ...
    all_memories AS (
        -- Temporarily filtering ONLY challenges for testing

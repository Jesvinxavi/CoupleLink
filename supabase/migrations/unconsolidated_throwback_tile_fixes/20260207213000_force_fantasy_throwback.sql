-- Migration: Force Fantasy Throwbacks (Testing)
-- Retains Challenge Media Aggregation fix
-- Forces return of only 'fantasy' type memories
-- NOW INCLUDES: fantasy_bucket_list items
...
BEGIN
    ...
    WITH all_memories_raw AS (
        ...
        SELECT 
            ...
            m.caption as content,
            m.created_at,
            -- AGGREGATED MEDIA (Keep this fix)
            (
        ...
        UNION ALL
        -- Fantasy Bucket List (Approved/Completed)
        SELECT 
            fbl.id,
    ...
    all_memories AS (
        -- CHANGED: Force filtering to ONLY 'fantasy' type
        SELECT * FROM all_memories_raw amr WHERE amr.type = 'fantasy'
    ),
    numbered_items AS (
        SELECT 
            am.*, 
